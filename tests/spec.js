...
// var keys = Object.keys(vm.object)
    ...
    describe("$watch对象的子属性", function () {
        it("async", function (done) {
            ...
            if (avalon.directive) {
                vm.$watch('postData.venue', function (val) {
                    a = val
                })
            } else {
...
bbb", "aaa,bbb")
    ...
    describe("avalon1.5之 前需要对计算属性与$model做的测试 ", function () {
        ...
        describe("计算属性", function () {
            ...
            it("async2", function (done) {
                ...
                setTimeout(function () {
                    ...
                    buttons[0].click()
                    expect(vm.test0).to.be(true)
                    expect(vm.test1).to.be(true)
                    setTimeout(function () {
                        buttons[1].click()
                        expect(vm.test0).to.be(false)
                        expect(vm.test1).to.be(false)
                        clearTest(vm, div, done)
            ...
            it("async3", function (done) {
                ...
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        expect(vm.test0).to.be(true)
                        expect(vm.test1).to.be(true)
                        expect(vm.test2).to.be(true)
                        expect(vm.msg).to.be("ok！！")
                        clearTest(vm, div, done)
                    })
                }, 100)
            ...
            it("多个依赖变动时，延迟计算属性的$watch回调", function (done) {
                ...
                } else if (avalon.directive && !avalon.config.async) {
        ...
        describe("双工绑定与$model", function () {
            ...
            it("async", function (done) {
                ...
                vm.$watch("selected", function (a, b) {
                    expect(vm.$model.selected).to.be("bbb")
                    expect(a).to.be("bbb")
                    expect(b).to.be("ddd")
        ...
        describe("对象循环的对象被整个替换掉BUG #955", function () {
            it("async", function (done) {
                var vm = avalon.define({
                    $id: "test",
                    object: {
            ...
            it("async", function (done) {
                ...
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        ...
                        var data = avalon.directive ?
                                vmodel.$events.arr[0] :
                                vmodel.arr.$events[avalon.subscribers][0]
                        ...
                        if (is138) {
                            ...
                            expect($proxies[0].el).to.be(vmodel.arr[0])
                            expect($proxies[0].el.$model).to.eql(vmodel.arr[0].$model)
                        }

                        expect(vmodel.$model.arr[0]).to.eql(vmodel.arr[0].$model)
    ...
    describe("对于不存在的属性将不移除对应的插值表达式或绑定属性", function () {
        ...
        it("async", function (done) {
            ...
            div.innerHTML = heredoc(function () {
                /*
                 <div>我的{{name}}叫{{answer}},他的{{name}}叫{{no}},{{10*10}}</div>
                 <p ms-text="name"></p> <p ms-text="no"></p>
                 */
            })
            body.appendChild(div)
            avalon.scan(div, vm)
            ...
            setTimeout(function () {
                ...
                expect(String(pp[0].getAttribute("ms-text"))).to.be("null")
                expect(String(pp[1].getAttribute("ms-text"))).to.be("no")
                clearTest(vm, div, done)
            })
        })
    ...
    describe("双工绑定ms-duplex-boolean", function () {
        ...
        it("async", function (done) {
            ...
            div.innerHTML = heredoc(function () {
                /*
                 <input ms-duplex-boolean="aaa" type="radio" value="true">
    ...
    describe("ms-with", function () {
        it("async", function (done) {
            ...
            div.innerHTML = heredoc(function () {
                /*
                 <div ms-with="aaa"><p>{{$key}}--{{$val}}</p><input ms-duplex="$val"/></div>
            ...
            setTimeout(function () {
                ...
                expect(ps[3].innerHTML).to.be("a--4")
                expect(ps[4].innerHTML).to.be("b--5")
                expect(ps[5].innerHTML).to.be("c--6")
                var inputs = div.getElementsByTagName("input")
                inputs[0].value = 10
                inputs[1].value = 20
                inputs[2].value = 30
    ...
    describe('$remove', function () {
        ...
        it("async", function (done) {
            ...
            div.innerHTML = heredoc(function () {
                /*
                 <ul>
                 <li ms-repeat="array"><button type="button" ms-click="$remove">{{el}}</button></li>
                 </ul>
                 */
            })
            body.appendChild(div)
